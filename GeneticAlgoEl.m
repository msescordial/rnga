function [S1] = GeneticAlgoEl(population_size, S0, S0r, maxiter, n, DistanceMatrix, TravelDemandMatrix, TimeMatrix, BusRouteID, TotalNoOfRoutes, s, transfer_time)
    s = length(S0); 
    
    init_pop_matrix = cell(population_size,3);
    
    % Creating the Initial Population 
    % 1st col: no., 2nd col: pop, 3rd col: Obj Func Value, 4th col: Fitness Value
    % First Row
    ...
    % Elite Percentage ep
    ep = 0.10;
    elite_pop = zeros(population_size*ep, s);
    for j=1:population_size*ep
        elite_pop(j,:)=mysorted{1,j};
    end
    %disp("Elite Population:"); disp(elite_pop);

    % Non-Elite Population
    non_elite_pop = zeros(population_size*(1-ep), s);
    for j=1:population_size*(1-ep)
        non_elite_pop(j,:)=mysorted{1,(population_size*ep)+j};
    end
    %disp("Non-Elite Population:"); disp(non_elite_pop);

    % Combine elite and non-elite
    initial_pop = zeros(population_size, s);
    for m=1:population_size*ep
        initial_pop(m,:) = elite_pop(m,:);
    end
    for m=1:population_size*(1-ep)
        initial_pop(population_size*ep+m,:) = non_elite_pop(m,:);
    end
    %disp("Initial Population:"); disp(initial_pop);
        
    % SOLUTION REPRESENTATION FOR GA 
    len_string = s*n;
    solution_s0 = zeros(population_size,len_string);

    % Pass Elite to the Next Generation
    %for h=1:population_size*0.5
    for h=1:population_size*ep
        y1=1; y2=n;
        for k=1:s
            route_string = BusRouteID{elite_pop(h,k),2};
            solution_s0(h,y1:y2)=route_string;
            y1=y1+n; y2=y2+n;
        end
    end
    %disp("Solution S0"); disp(solution_s0);

    % Current Generation: 
    curr_solution_s0 = zeros(population_size,len_string);
    for h=1:population_size
        y1=1; y2=n;
        for k=1:s
            route_string = BusRouteID{initial_pop(h,k),2};
            curr_solution_s0(h,y1:y2)=route_string;
            y1=y1+n; y2=y2+n;
        end
    end
    %disp("Current Solution S0"); disp(curr_solution_s0);
    ...
    if (intercrossover == 1)
    % 1. crossover_matrix has size pop_size*(1-ep) but still includes elite_pop for crossover, 
    % solution_s0 = elite_pop + crossover_matrix
    crossover_matrix = zeros(population_size*(1-ep),len_string);

    % Random pairing of parent solutions for crossover
    r = randperm(population_size);      
    for m=1:population_size*(1-ep)
        crossover_matrix(m,:) = curr_solution_s0(r(m),:);
    end
    ...
    after_intercross = zeros(population_size*(1-ep),len_string);
    for s1=1:population_size*0.5*(1-ep)
    ...
    %disp("After Inter-Crossover"); %disp(after_intercross);
    %displayPopulationSolution(after_intercross,n);
    ...
    if (intracrossover == 1)
        if (intercrossover == 1) 
            ...
            crossover_matrix2 = solution_s0(1:population_size,:);
        end
        
        after_intracross = zeros(population_size*(1-ep),len_string);
        
        % 1. & 3. Choosing Parent population randomly
        for s2=1:population_size*(1-ep)
    ...
    after_mutation = zeros(population_size*(1-ep), s*n);
    P_m = 0.05;            % mutation probability
    for d=1:population_size*(1-ep)
        d1 = rand();      
        if (d1 < P_m)
    ...
    new_generation = solution_s0(1:population_size*ep,:);
    hp = population_size*ep;
    ...
    new_generation(hp+1:population_size,:) = after_mutation;
